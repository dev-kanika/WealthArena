...
DDL = r"""
IF OBJECT_ID('dbo.fx_processed_prices','U') IS NULL
CREATE TABLE dbo.fx_processed_prices (
  symbol         VARCHAR(20)       NOT NULL,
  ts_local       DATETIMEOFFSET(0) NOT NULL, -- FX local time (UTC in this pipeline)
  ...
  CONSTRAINT PK_fx_processed_prices PRIMARY KEY CLUSTERED (symbol, date_utc)
);

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='IX_fx_processed_date')
  CREATE INDEX IX_fx_processed_date
  ON dbo.fx_processed_prices(date_utc)
  INCLUDE (symbol,[close],volume);

IF OBJECT_ID('dbo.fx_processed_stage','U') IS NULL
CREATE TABLE dbo.fx_processed_stage (
  symbol         VARCHAR(20)       NOT NULL,
