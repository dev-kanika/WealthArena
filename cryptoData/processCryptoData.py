...
RAW_DIRS = [
    BASE_DIR / "data" / "raw_crypto",     # from the crypto downloader you ran
    BASE_DIR / "data" / "crypto" / "raw", # alternate layout if you change later
]

DATA_OUT_DIR     = BASE_DIR / "data" / "crypto"
...
# For timestamp presentation; crypto is 24/7 so this is a label only
...
def process_one_local(csv_path: Path) -> Tuple[str, pl.DataFrame]:
    """
    Read a crypto *_raw.csv (columns: Open,High,Low,Close,Volume,Date),
    compute indicators, and return processed Polars DataFrame.
    """
    name = csv_path.name
    symbol = name.replace("_raw.csv", "").upper()     # e.g., BTC-AUD or BTC-USD
    ...
    df = (
        df.with_columns([
            ...
            pl.col("Volume").cast(pl.Float64, strict=False),  # crypto volumes can be fractional
    ...
    out = (
        ...
        .with_columns([
            (pl.col("Returns").rolling_std(20) * math.sqrt(365)).alias("Volatility_20"),
...
def save_processed_per_symbol(symbol: str, df_pl: pl.DataFrame) -> Path:
    out_path = PROCESSED_DIR / f"{symbol}_processed.csv"
    df_pl.write_csv(out_path)
    return out_path

def save_parquet_per_symbol(symbol: str, df_pl: pl.DataFrame) -> Path:
    pq_path = PARQUET_DIR / f"{symbol}_processed.parquet"
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Preprocess local CRYPTO CSVs (no Azure, Polars, indicators).")
    parser.add_argument("--make-parquet", action="store_true",
                        help="Also write one Parquet per symbol to data/crypto/processed_parquet/")
    parser.add_argument("--max-workers", type=int, default=MAX_WORKERS)
    args = parser.parse_args()

    paths = list_local_raw()
    print(f"Found {len(paths)} RAW crypto file(s) under:")
    for r in RAW_DIRS:
        if r.exists():
            print(f"  - {r}")

    if not paths:
        print("No *_raw.csv files found. Put your raw crypto CSVs in one of the folders above.")
        return
    ...
    with ThreadPoolExecutor(max_workers=int(args.max_workers)) as ex:
        ...
        for i, fut in enumerate(as_completed(futs), 1):
            ...
            out_csv = save_processed_per_symbol(symbol, dfp)
            outputs.append(out_csv)
            written_rows += dfp.height

            if args.make_parquet:
                pq = save_parquet_per_symbol(symbol, dfp)
