...
# NOTE: separate prefix for FX; defaults to asxForex
PREF = os.getenv("ADLS_RAW_PREFIX_FOREX", "asxForex")
...
TRUNCATE_PRICES_BEFORE = os.getenv("TRUNCATE_FX_BEFORE", "false").strip().lower() in {"1", "true", "yes"}
...
# Destination timezone for “local” timestamps — for FX we keep UTC
FX_TZ = "UTC"
...
def list_paths() -> list[str]:
    # FX files end with *_fx_raw.csv (e.g., AUDUSD_fx_raw.csv)
    paths = fs.glob(f"abfs://{CONT}/{PREF}/*_fx_raw.csv")
    paths.sort()
    return paths
...
def process_one(abfs_path: str) -> tuple[str, pl.DataFrame]:
    ...
    # Normalize columns from the downloader (Open,High,Low,Close,Volume,Date)
    ...
    df = (
        df.with_columns([parse_datetime_utc("Date").alias("ts_utc")])
          .drop("Date")
          .with_columns([
              pl.col("ts_utc").dt.convert_time_zone(FX_TZ).alias("ts_local"),
...
def insert_stage(conn, df_pl: pl.DataFrame) -> int:
    """Insert rows into dbo.fx_processed_stage in batches."""
    if df_pl.is_empty():
        return 0
    ...
    for start in range(0, len(rows), BATCH_ROWS):
        ...
        cur.executemany("""
            INSERT INTO dbo.fx_processed_stage (
                symbol,ts_local,date_utc,[open],[high],[low],[close],volume,
                sma_5,sma_10,sma_20,sma_50,sma_200,
                ema_12,ema_26,rsi_14,
                macd,macd_signal,macd_hist,
...
def merge_stage(conn) -> tuple[int, int]:
    """MERGE stage -> fx_processed_prices, return (inserted, updated), then TRUNCATE stage."""
    cur = conn.cursor()
    cur.execute("SET LOCK_TIMEOUT 60000;")

    cur.execute("SELECT DB_NAME();")
    dbname = cur.fetchone()[0]
    print(f"[DB] FX MERGE running on database: {dbname}")

    cur.execute("IF OBJECT_ID('tempdb..#act') IS NOT NULL DROP TABLE #act; "
                "CREATE TABLE #act(action NVARCHAR(10));")

    cur.execute("""
        MERGE dbo.fx_processed_prices AS tgt
        USING (SELECT * FROM dbo.fx_processed_stage) AS src
    ...
    print(f"FX MERGE summary -> inserted: {inserted or 0}, updated: {updated or 0}")

    cur.execute("DROP TABLE #act;")
    cur.execute("TRUNCATE TABLE dbo.fx_processed_stage;")
    conn.commit()
...
def main() -> None:
    ...
    paths = list_paths()
    print(f"Found {len(paths)} FX RAW files under abfs://{CONT}/{PREF}/")
    if not paths:
        return
    ...
    if TRUNCATE_PRICES_BEFORE:
        print("⚠️  TRUNCATE dbo.fx_processed_prices (full reload requested)...")
        cur.execute("TRUNCATE TABLE dbo.fx_processed_prices;")
    conn.commit()
        ...
        if DELETE_SOURCE:
            ...
            for p in pending_to_delete:
                if _rm_with_retries(p):
                    ok += 1
            print(f"Deleted {ok}/{len(pending_to_delete)} FX source file(s) from ADLS (post-MERGE).")
        pending_to_delete = []
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in as_completed(futures):
                ...
                if staged_rows >= MERGE_EVERY_ROWS:
                    print(f"FX MERGE after staging ~{staged_rows} rows …")
                    do_merge_and_delete()
                    staged_rows = 0

                if processed_files % 100 == 0:
                    print(f"Processed {processed_files}/{len(paths)} FX files …")
